...
def read_data(tile_id, tile_info, data_dir, img_size, exisiting_datasets=None):
    ...
    for modality, modality_info in MODALITIES.items():
        if (
            exisiting_datasets is not None
            and modality not in exisiting_datasets
        ):
            continue
        ...
        if modality == "sentinel2":
            ...
            count += (
                len(tile_info_bands["sentinel2"]) - 3
                if type == "l2a"
                else len(tile_info_bands["sentinel2"]) - 1
            )
            for i, band in enumerate(sentinel2_bands):
                if band in tile_info_bands["sentinel2"]:
                    placeholder[i] = np.expand_dims(
                        data[:, :, tile_info_bands["sentinel2"].index(band)], 2
                    ).transpose(2, 0, 1)
            return_data_dict["sentinel2"] = placeholder

        if modality == "sentinel2_cloudmask":
            if "QA60" in tile_info_bands["sentinel2"]:
                return_data_dict["sentinel2_cloudmask"] = np.expand_dims(
                    data[:, :, tile_info_bands["sentinel2"].index("QA60")], 2
                ).transpose(2, 0, 1)
                count += 1
            else:
                return_data_dict["sentinel2_cloudmask"] = (
                    np.ones((1, img_size, img_size), dtype="uint16") * 65535
                )
        ...
        if modality == "sentinel2_cloudprod":
            ...
            if "MSK_CLDPRB" in tile_info_bands["sentinel2"]:
                count += 1
                return_data_dict["sentinel2_cloudprod"] = np.expand_dims(
                    data[
                        :, :, tile_info_bands["sentinel2"].index("MSK_CLDPRB")
                    ],
                    2,
                ).transpose(2, 0, 1)
            else:
                return_data_dict["sentinel2_cloudprod"] = (
                    np.ones((1, img_size, img_size), dtype="uint16") * 65535
                )
        ...
        if modality == "sentinel2_scl":
            if "SCL" in tile_info_bands["sentinel2"]:
                count += 1
                return_data_dict["sentinel2_scl"] = np.expand_dims(
                    data[:, :, tile_info_bands["sentinel2"].index("SCL")], 2
                ).transpose(2, 0, 1)
            else:
                return_data_dict["sentinel2_scl"] = (
        ...
        if modality == "sentinel1":
            ...
            tmp = np.ones((8, img_size, img_size), dtype="float32") * float(
                "-inf"
            )
            for orbit in ["asc", "desc"]:
                if tile_info_bands[f"sentinel1_{orbit}"] is not None:
                    ...
                    for i, band in enumerate(bands_img):
                        tmp[orbit_map[orbit] + bands_map[band]] = data[
                            :, :, count - len(bands_img) + i
                        ]
        ...
        if modality == "aster":
            ...
            return_data_dict["aster"] = data[
                :, :, count - len(tile_info_bands["aster"]) : count
            ].transpose(2, 0, 1)
        ...
        if modality == "era5":
            ...
            return_data_dict["era5"] = np.stack(
                [era_data["month1"] + era_data["month2"] + era_data["year"]],
                axis=0,
            ).astype("float32")
        ...
        if modality == "dynamic_world":
            try:
                ...
                return_data_dict["dynamic_world"] = np.expand_dims(
                    data[:, :, count - len(tile_info_bands["dynamic_world"])],
                    axis=0,
                )
            except Exception as e:
                print("dynamic_world not found")
                return_data_dict["dynamic_world"] = np.zeros(
                    (1, img_size, img_size), dtype="uint8"
                )
        ...
        if modality == "canopy_height_eth":
            ...
            tmp = data[
                :, :, count - len(tile_info_bands["canopy_height_eth"]) : count
            ].transpose(2, 0, 1)
            if tmp.shape[0] == 0:
                return_data_dict["canopy_height_eth"] = (
                    np.ones((2, img_size, img_size), dtype="int8") * 255
                )
            else:
                return_data_dict["canopy_height_eth"] = data[
                    :,
                    :,
                    count - len(tile_info_bands["canopy_height_eth"]) : count,
                ].transpose(2, 0, 1)
        if modality == "copernicus_dem":
            try:
                count += len(tile_info_bands["copernicus_dem"])
                return_data_dict["copernicus_dem"] = np.expand_dims(
                    data[:, :, count - len(tile_info_bands["copernicus_dem"])],
                    axis=0,
                )
            except Exception as e:
                print("copernicus_dem not found")
                return_data_dict["copernicus_dem"] = np.zeros(
                    (1, img_size, img_size), dtype="float32"
                )

        ### LATITUDE ###
        if modality == "lat":
            return_data_dict["lat"] = np.stack(
                [
                    np.sin(np.deg2rad(tile_info["lat"])),
                    np.cos(np.deg2rad(tile_info["lat"])),
                ],
                axis=0,
            ).astype("float32")
        ...
        ### LONGITUDE ###
        if modality == "lon":
            return_data_dict["lon"] = np.stack(
                [
                    np.sin(np.deg2rad(tile_info["lon"])),
                    np.cos(np.deg2rad(tile_info["lon"])),
                ],
                axis=0,
            ).astype("float32")
        ...
        if modality == "month":
            ...
            month = int(month)
            return_data_dict["month"] = np.stack(
                [
        ...
        if modality == "esa_worldcover":
            try:
                ...
                return_data_dict["esa_worldcover"] = np.expand_dims(
                    data[:, :, count - len(tile_info_bands["esa_worldcover"])],
                    axis=0,
                )
                ...
                print("exception: ", e)
                exit()
                return_data_dict["esa_worldcover"] = (
                    np.ones((1, img_size, img_size), dtype="uint8") * 255
                )
...
def main(args):
    ...
    if mode == "create":
        ...
        hdf5_file = h5py.File(args.output_file, "w")
        ...
        print(
            "Number of tiles skipped due to mismatch: ",
            len(tile_info) - num_tiles,
        )
        # exit()
        ...
        for modality, modality_info in MODALITIES.items():
            if modality == "lat" or modality == "lon" or modality == "month":
                variables[modality] = hdf5_file.create_dataset(
                    modality, shape=(num_tiles, 2), dtype=modality_info["dtype"]
                )
            elif modality == "biome":
                variables[modality] = hdf5_file.create_dataset(
                    modality,
                    shape=(num_tiles, 14),
                    dtype=modality_info["dtype"],
                )
                ...
                variables[modality] = hdf5_file.create_dataset(
                    modality,
                    shape=(num_tiles, modality_info["n_bands"]),
                    dtype=modality_info["dtype"],
                )
            else:
                variables[modality] = hdf5_file.create_dataset(
        ...
        ds_metadata = hdf5_file.create_dataset(
            "metadata", shape=(num_tiles,), dtype=metadata_dt
        )
        ...
        for i, tile_id in enumerate(tile_info):
            ...
            for b in [
                ...
                if b in tile_info[tile_id]["BANDS"]:
                    count_t += (
                        len(tile_info[tile_id]["BANDS"][b])
                        if tile_info[tile_id]["BANDS"][b] is not None
                        else 0
                    )
        ...
        for modality, modality_info in MODALITIES.items():
            if modality == "lat" or modality == "lon" or modality == "month":
                variables[modality] = hdf5_file.create_dataset(
                    modality, shape=(size, 2), dtype=modality_info["dtype"]
                )
            elif modality == "biome":
                variables[modality] = hdf5_file.create_dataset(
                    modality, shape=(size, 14), dtype=modality_info["dtype"]
                )
            elif modality == "eco_region":
                variables[modality] = hdf5_file.create_dataset(
                    modality, shape=(size, 846), dtype=modality_info["dtype"]
                )
            elif modality == "era5":
                variables[modality] = hdf5_file.create_dataset(
                    modality,
                    shape=(size, modality_info["n_bands"]),
                    dtype=modality_info["dtype"],
                )
            else:
                variables[modality] = hdf5_file.create_dataset(
                    modality,
                    shape=(size, modality_info["n_bands"], img_size, img_size),
                    dtype=modality_info["dtype"],
                )

        metadata_dt = np.dtype([("tile_id", "S100")])  # string of length 100
        ds_metadata = hdf5_file.create_dataset(
            "metadata", shape=(size,), dtype=metadata_dt
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert the data to h5 format"
    )
    parser.add_argument(
        "--mode",
        type=str,
        required=True,
        help="append or create",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="path to the tile info json file",
    )
    parser.add_argument(
        "--output_file", type=str, default="", help="path to the output h5 file"
    )
    parser.add_argument(
        "--missing_tiles",
        type=str,
        default="",
        help="path to the csv file containing the missing tiles",
    )
    parser.add_argument(
        "--image_size", type=int, default=128, help="size of the image"
    )

    # args for merge mode
    # required args
    parser.add_argument(
        "--data_dir1", type=str, default="", help="path to the first folder"
    )
    parser.add_argument(
        "--data_dir2", type=str, default="", help="path to the first folder"
    )
    parser.add_argument(
        "--output_path", type=str, help="path to the output h5 file"
    )
    # optional args
    parser.add_argument(
        "--path1", type=str, default="", help="path to the first h5 file"
    )
    parser.add_argument(
        "--path1_tile_info",
        type=str,
        default="",
        help="path to the tile info json file for the first h5 file",
    )
    parser.add_argument(
        "--path2", type=str, default="", help="path to the second h5 file"
    )
    parser.add_argument(
        "--path2_tile_info",
        type=str,
        default="",
        help="path to the tile info json file for the second h5 file",
    )
    ...
    if args.mode == "merge":
        ...
        assert (
            args.data_dir1 != ""
        ), "Please provide the path to the first folder"
        assert (
            args.data_dir2 != ""
        ), "Please provide the path to the second folder"

        name1 = (
            args.data_dir1.split("/")[-1]
            if args.data_dir1[-1] != "/"
            else args.data_dir1.split("/")[-2]
        )
        name2 = (
            args.data_dir2.split("/")[-1]
            if args.data_dir2[-1] != "/"
            else args.data_dir2.split("/")[-2]
        )
        if args.path1 == "":
            args.path1 = os.path.join(args.data_dir1, name1 + ".h5")
        if args.path1_tile_info == "":
            args.path1_tile_info = os.path.join(
                args.data_dir1, name1 + "_tile_info.json"
            )
        if args.path2 == "":
            args.path2 = os.path.join(args.data_dir2, name2 + ".h5")
        if args.path2_tile_info == "":
            args.path2_tile_info = os.path.join(
                args.data_dir2, name2 + "_tile_info.json"
            )
    ...
    if args.mode == "create":
        ...
        name = (
            ...
            if args.data_dir[-1] != "/"
            else args.data_dir.split("/")[-2]
        )
        if args.tile_info == "":
            args.tile_info = os.path.join(
                args.data_dir, name + "_tile_info.json"
            )
        if args.output_file == "":
            args.output_file = os.path.join(args.data_dir, name + ".h5")
        if args.missing_tiles == "":
            args.missing_tiles = os.path.join(
                args.data_dir, name + "missing_tiles.csv"
            )
